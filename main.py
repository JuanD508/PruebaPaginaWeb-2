...
def datos():
    return '<h1>Datos Curiosos</h1>'f'<p>{random.choice(facts_list)}</p> <a href="/">Volver al menu</a>'

@app.route("/")
def index():
    return f'<h1>Dependencias Tecnologicas</h1> <a href="/random_fact">¡Ver un dato aleatorio!</a> <a href="/coins">¡Lanzar una moneda!</a>'

@app.route("/coins")
def coins():
    return f'<h1>Lanzar Moneda</h1><p>{random.choice(coin)}</p> <a href="/">Volver al menu</a>'
app.run(debug=True)
